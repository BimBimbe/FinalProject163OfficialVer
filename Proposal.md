...
URL: https://water.usgs.gov/nawqa/pnsp/usage/maps/county-level/ (year-by-year Preliminary Estimates files, 2013-2017)
This dataset contains estimated annual agricultural pesticide-use data at the county level for the contiguous United States from 2013-2017. We use USGS's year-by-year "Preliminary Estimates" releases rather than the later ver. 2.0 consolidated ScienceBase release (DOI 10.5066/P9F2SRYH) originally identified during proposal planning, because that ScienceBase release was unreachable (repeated server outages) at the time we needed to download it. Both releases share the same variables (COMPOUND, YEAR, STATE_FIPS_CODE, COUNTY_FIPS_CODE, EPEST_LOW_KG, EPEST_HIGH_KG), years, and underlying methodology; the ver. 2.0 release differs only in that it revised estimates for a subset of compounds with missing data. We will clean and aggregate estimated pesticide use across counties and years to create a state-level pesticide-use measure, used in Research Questions 1 and 2. This release covers only the conterminous U.S., so Alaska and Hawaii are excluded, and California is excluded entirely from the 2017 file (USGS distributes 2017 as a separate "NoCA" file, with California's values normally coming from a separate state source appended after the fact); we do not include a separate California source given our project scope. Seed-treatment applications were dropped starting in 2015, creating a small methodological break between 2013-14 and 2015-17.

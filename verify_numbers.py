"""Verification script: re-run all analyses and extract numbers that
must match report prose.
...
print(
    f"  pesticide_intensity mean: {q1_pesticide_mean:.3f} "
    "(report says: 0.706)"
)
...
print(
    f"  pesticide_intensity mean: {q2_pesticide_mean:.3f} "
    "(report says: 0.706)"
)
print(
...
q2_df['interaction'] = (
    q2_df['pesticide_intensity'] * q2_df['nitrate_rate_per_system']
)
X3 = sm.add_constant(
    q2_df[['pesticide_intensity', 'nitrate_rate_per_system', 'interaction']]
)
...
print(
    f"  colorectal_rate mean: {q3_colorectal_mean:.2f} "
    "(report says: 43.32)"
)
...
hormone_stats = state_table['hormone_rate'].describe(
    percentiles=[.25, .5, .75]
).round(2)
pesticide_stats = state_table['pesticide_intensity'].describe(
    percentiles=[.25, .5, .75]
).round(2)
overall_stats = state_table['overall_rate'].describe(
    percentiles=[.25, .5, .75]
).round(2)
nitrate_stats = state_table['nitrate_rate_per_system'].describe(
    percentiles=[.25, .5, .75]
).round(4)
colorectal_stats = state_table_cdc_epa['colorectal_rate'].describe(
    percentiles=[.25, .5, .75]
).round(2)
stomach_stats = state_table_cdc_epa['stomach_rate'].describe(
    percentiles=[.25, .5, .75]
).round(2)
...
summary_table = pd.DataFrame({
    ...
    'Mean': [
        hormone_stats['mean'], pesticide_stats['mean'],
        overall_stats['mean'], nitrate_stats['mean'],
        colorectal_stats['mean'], stomach_stats['mean'],
    ],
    'SD': [
        hormone_stats['std'], pesticide_stats['std'],
        overall_stats['std'], nitrate_stats['std'],
        colorectal_stats['std'], stomach_stats['std'],
    ],
    'Min': [
        hormone_stats['min'], pesticide_stats['min'],
        overall_stats['min'], nitrate_stats['min'],
        colorectal_stats['min'], stomach_stats['min'],
    ],
    'Q1': [
        hormone_stats['25%'], pesticide_stats['25%'],
        overall_stats['25%'], nitrate_stats['25%'],
        colorectal_stats['25%'], stomach_stats['25%'],
    ],
    'Median': [
        hormone_stats['50%'], pesticide_stats['50%'],
        overall_stats['50%'], nitrate_stats['50%'],
        colorectal_stats['50%'], stomach_stats['50%'],
    ],
    'Q3': [
        hormone_stats['75%'], pesticide_stats['75%'],
        overall_stats['75%'], nitrate_stats['75%'],
        colorectal_stats['75%'], stomach_stats['75%'],
    ],
    'Max': [
        hormone_stats['max'], pesticide_stats['max'],
        overall_stats['max'], nitrate_stats['max'],
        colorectal_stats['max'], stomach_stats['max'],
    ],
})
...
for name, reported, computed, tolerance in discrepancies:
    ...
    print(
        f"{status:8s} {name:30s}: reported={reported:8.4f}, "
